...
async function Header({ borderClassName }: { borderClassName?: string }) {
    const session = await auth();

    return (
        <nav
            className={`fixed left-0 right-0 top-0 z-[1001]   flex  h-[62px] w-full    flex-shrink-0  flex-row  items-center   justify-center    bg-white  px-4 mobile:px-6 ${borderClassName} `}
        >
