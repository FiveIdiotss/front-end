...
import DotLoading from '../../_component/icon/DotLoadingIcon';
    ...
    return (
        ...
        >
            ...
            <SectionDivider
                ...
                color={`${isFocused ? 'border-indigo-500' : 'border-neutral-300'}`}
            />
            <button className="flex h-full w-11 cursor-pointer items-center justify-center">
                {!loading && (
                    <SearchIcon className={`h-5 w-5 ${isFocused ? 'rotate-90 text-indigo-500' : 'text-neutral-300'}`} />
                )}
                {loading && (
